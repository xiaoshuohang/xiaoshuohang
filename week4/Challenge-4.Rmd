...
```{r, eval=TRUE,echo=TRUE}
# Enter code here
comm_data%>%filter(sentiment>0)%>%group_by(sender)%>%summarise(x=mean(sentiment))%>%arrange(desc(x))%>%slice(1:3)
```
<br>
...
```{r, eval=TRUE,echo=TRUE}
# Enter code here
comm_data%>%group_by(date)%>%summarise(avarge_score=mean(sentiment))%>%arrange(date)
```
<br>
...
```{r, eval=TRUE,echo=TRUE}
# Enter code here
comm_data %>%mutate(sentiment_label = ifelse(sentiment> 0, "Positive",
...
```{r, eval=TRUE,echo=TRUE}
# Enter code here
comm_data%>%group_by(date)%>%summarise(total_chars_sent = sum(nchar(message)))%>%arrange(desc(total_chars_sent))%>%slice(1)
```
<br>
...
**Solution:** _it is a untidy data because it has multiple variables in columns ,and it contains NA.Can change the name of the row to column_